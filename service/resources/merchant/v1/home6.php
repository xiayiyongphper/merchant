...
class home6 extends MerchantResourceAbstract
{
    protected $_areaId;
    protected $_cityId;
    ...
    {
        $key = 'merchant_home_page_v6_' . $this->_areaId;
        //APP与PC的首页返回区分
...
//        Tools::log($quickEntryBlocks,'wangyang.log');
    ...
    protected function processStore()
    {
        ...
        if ($banner) {
            ...
            $store_block_title_img = 'http://assets.lelai.com/images/files/merchant/20170122/source/0_20170122141217file.png?width=640&height=98';
        }
        ...
        //推荐供应商，供应商权重排序
        $recommendWholesalerIds = self::getWholesalerIdsByMerchantCategory($this->_areaId, $wholesalerCount);
        $wholesalers = self::getStoreDetailBrief($recommendWholesalerIds, $this->_areaId);

        $store_block['store_list'] = $wholesalers;
        //全部店铺数量
		...
		// remark
		if(isset($this->_configDate['store']['remark'])){
			$store_block['remark'] = $this->_configDate['store']['remark'];
		}
        //block上方图片
