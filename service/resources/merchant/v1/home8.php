...
class home8 extends MerchantResourceAbstract
{
    protected $_customer;
    protected $_areaId;
    protected $_cityId;
    ...
    public function run($data)
    {
        ...
        $this->toArray();
//        Tools::log(Tools::pb_array_filter($this->_data), 'home8.log');
...
//                if(!empty($topic_block['area_ids']) && !in_array($this->_areaId,$topic_block['area_ids'])){
...
//    }
    ...
    {
        $key = 'merchant_home_page_v8_' . $this->_areaId;
        ...
        if (false && $this->getRedisCache()->exists($key)) {
            ...
            Tools::log($this->_configDate, 'hl.log');
...
//        Tools::log($brand_data, 'wangyang.log');
    ...
    /**
     * 秒杀模块。
     * 先判断有没有合适的活动，再判断有没有配送区域的商品
     * @return $this|void
     */
    protected function getSecKillBlock()
    {
...
//        Tools::log($banner,'home8.log');
    ...
    protected function getHomeBanner()
    {
        ...
        $banners = LeBanner::find()->where(
            ...
            ->andWhere(['>=', 'end_date', $now])
            ->orderBy('sort desc');
//        Tools::log($banners->createCommand()->getRawSql(),'home8.log');
        $banners = $banners->asArray()->all();
        if (count($banners) > 0) {
            foreach ($banners as $item) {
                //banner判断是否属于该用户分群
...
//                Tools::log($group_to_show,'home8.log');
    ...
    protected function getProductBlock()
    {
        ...
        foreach ($product_blocks as $product_block) {
            ...
            //Tools::log($product_block,'hl.log');
    ...
    protected function processStore()
    {
        ...
        Tools::log("all_merchant_store_category=====", 'hl.log');
        Tools::log($all_merchant_store_category, 'hl.log');
