 ...
 * Time: 下午1:38
 */
abstract class AlipayAbstract extends VarienObject implements PaymentInterface
{
    protected $_logFile = 'alipay.log';
    ...
    protected function sendRequest($url, $data = array(), $action = 'POST')
    {
        ...
        $output = curl_exec($ch);
        if ($output == false) {
            $this->logException(new LE_Alipay_Model_Exception(curl_error($ch)));
        } else {
            $this->log($output);
        }
        return $output;
    ...
    protected function getConfig($path, $throwExceptionIfNull = false, $storeId = null)
    {
        ...
        if ($throwExceptionIfNull && is_null($value)) {
            throw new LE_Alipay_Model_Exception(sprintf('Config path:%s value is null', $path));
        }
        return $value;
    }
}